...
library DateTime {
    ...
    function formatYYYYMMDD(uint256 timestamp) internal pure returns (string memory) {
        uint256 _year = getYear(timestamp);
        uint256 _month = getMonth(timestamp);
        uint256 _day = getDay(timestamp);

        string memory year = _year.toString();
        string memory month = _month > 9 ? _month.toString() : string(abi.encodePacked("0", _month.toString()));
        string memory day = _day > 9 ? _day.toString() : string(abi.encodePacked("0", _day.toString()));
